  ...
  return (
    <View className="flex-row gap-2.5 px-4 pb-4">
      <Pressable
        ...
        className="flex-1 items-center justify-center rounded-2xl bg-ink p-3.5"
      >
        <Text className="text-[13px] font-bold text-white">📞 Contact & socials</Text>
      </Pressable>
      <Pressable
        onPress={() => setOpenAction('hours')}
        className="flex-1 items-center justify-center rounded-2xl bg-sand p-3.5"
      >
      ...
      <BottomSheet visible={openAction !== null} onClose={() => setOpenAction(null)}>
        {openAction === 'contact' ? (
          <RedirectOptionsSheetContent
            ...
            options={[
              { icon: '📞', label: `Ligar: ${phone}` },
              { icon: '💬', label: `WhatsApp: ${whatsapp}` },
              { icon: '📷', label: `Instagram: ${instagramHandle}` },
            ]}
          />
        ) : null}
