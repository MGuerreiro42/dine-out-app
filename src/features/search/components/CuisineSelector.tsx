  ...
  return (
    ...
    >
      {cuisines.map((cuisine) => (
        <Pressable key={cuisine.id} onPress={() => onSelect(cuisine.id)} className="w-16 items-center gap-1.5">
          <View
          ...
          <Text
            className={`text-center text-[13px] font-bold ${cuisine.isActive ? 'text-[#111827]' : 'text-[#6b7280]'}`}
          >
            {cuisine.label}
          </Text>
        </Pressable>
