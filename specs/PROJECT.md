...
**Status**: Restaurant data wired to a real backend (`dine-out-backend-overture`); auth/favorites real-backend integration designed, not implemented (`dine-out-backend`'s `AuthModule`/`FavoritesModule`) · **Last updated**: 2026-08-29
...
| `restaurant` | `specs/restaurant.md` | US1–US7 implemented; wired to real backend 2026-08-26 — price/amenities/opening-hours/description have no backend equivalent and render as correctly-empty states (see PROJECT.md decision log); rating/reviews now real on the detail screen only, sourced from `reviews.md` |
| `favorites` | `specs/favorites.md` | Global store (`stores/favorites.ts`), Profile rail (US2), and server-backed persistence (US3, `GET/PUT/DELETE /favorites`) all implemented and verified against the real backend |
| `reviews` | `specs/reviews.md` | Implemented |
| `profile` | `specs/profile.md` | US1+US2 implemented (mock data); US3-US6 not started |
| `auth` | `specs/auth.md` | US1, US3, US4, US5, US6 implemented; US2 true-by-construction; form validation (FR-010) implemented; real-backend integration (signup/login/refresh/logout against `dine-out-backend`'s `AuthModule`, `expo-secure-store`) implemented and verified against the real backend |
...
- **`src/stores/auth.ts`** (`status`, `user`, `accessToken`, `bootstrap()`, `setSession()`, `logout()`) is the second global store — read by the Sidebar and Profile's header/logout, same promotion rule as favorites. Real-backend integration (signup/login/refresh/logout against `dine-out-backend`, `expo-secure-store` for the refresh token) implemented and verified per `auth.md`'s 2026-08-29 pass.
- **`stores/auth.ts` and `stores/favorites.ts` import each other** (a require cycle, confirmed benign by a clean Metro bundle and passing tests) — `auth.ts` calls `favorites.ts`'s `setFavoriteIds([])` on logout/session-expiry, `favorites.ts` calls `auth.ts`'s `isLoggedIn` inside `toggleFavorite`. Both sides only use lazy `.getState()` calls, never a top-level import binding, which is what keeps the cycle safe.
- **`expo-secure-store` has no web implementation** (`ExpoSecureStore.web.ts` is `export default {}`) — every `SecureStore.*Async` call throws synchronously on `expo start --web`. `secureTokenStorage.ts` wraps all three functions in try/catch, treating a throw the same as "no stored token," so cold-start hydration and login/logout stay crash-free on web (same native-only-persistence shape as the existing `react-native-maps` limitation). The refresh token itself does not persist across a full page reload on web — expected, not a bug; verify persistence-across-restart (`auth.md` US5) on a native build.
...
- **`auth.md`/`favorites.md` real-backend integration designed (2026-08-29), not implemented** — `dine-out-backend`'s `AuthModule`/`FavoritesModule` are already built (per `ARCHITECTURE.md` §5/§6/§7); this pass designs the frontend wiring only. `src/lib/apiClient.ts` gains `apiPost`/`apiPut`/`apiDelete`, Bearer-header injection, and a deduped 401→refresh→retry-once interceptor, all internal to `apiClient.ts` (it does not import `src/stores/auth.ts`, avoiding a cycle — `auth.ts` registers a session-expired callback and mirrors its access token into `apiClient.ts`'s module state instead). New file `src/lib/secureTokenStorage.ts` is the sole `expo-secure-store` touchpoint, holding the refresh token — **`expo-secure-store` is a new dependency, not yet installed**; sign-off granted 2026-08-29 (`ARCHITECTURE.md` §5 already designs its use). `src/stores/favorites.ts` gains `setFavoriteIds`, populated from a new `GET /favorites`-backed query mounted app-wide from `app/_layout.tsx`; `toggleFavorite`/`isFavorite`'s call-site signatures are unchanged — `toggleFavorite` itself now guards on `isLoggedIn` and shows a login-prompt `Alert` when logged out, since guest favoriting is removed as a capability. Four open product decisions resolved 2026-08-29: guest favoriting removed (login-prompt `Alert`, nothing to migrate); cold-start refresh-token hydration (`bootstrap()`) fully replaces `isLoggedIn`'s static guest default; client-side password minimum raised to 8 to match the backend; signup `409`/login `401` render as inline per-field errors matching FR-010's pattern.
