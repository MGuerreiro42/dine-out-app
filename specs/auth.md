...
A user toggles between "Entrar" (login) and "Criar conta" (signup) modes on a single screen, and authenticates with email/password or a simulated Google/Apple flow.
...
5. **Given** login mode, **when** the user taps "Esqueceu a senha?," **then** a simulated response occurs.
...
4. **Given** `isLoggedIn` true, **when** the sidebar renders, **then** a "Sair da conta" control (danger styling) shows at the bottom; given false, it doesn't render.
...
- Sidebar omits "Formas de pagamento" (a Profile-only account-menu option).
...
- **FR-008** — Implemented: sidebar lists Home, Buscar, Categorias, Favoritos, Meus pedidos, Minhas reservas, Notificações — "Formas de pagamento" excluded.
...
- **FR-010** — Not implemented: `AuthForm` MUST validate on submit, before FR-004's side effect:
  - Email (both modes): required, basic format check. Empty → "Informe seu e-mail." Malformed → "E-mail inválido."
  - Password (both modes): required, min 6 chars. Empty → "Informe sua senha." Too short → "A senha deve ter pelo menos 6 caracteres."
  - Full name (signup only): required, non-empty after trim. Empty → "Informe seu nome completo."
  - Error text uses the same danger color as "Sair da conta"; field border switches to match.
...
- `favorites.md`'s favorites rail is reachable both from Profile and the sidebar's "Favoritos" entry (same route) — no extra wiring needed.
