...
**Created**: 2026-07-23
**Status**: In Progress — User Stories 1, 3, 4, 5, 6 Implemented (real backend); User Story 2 true by construction; form validation (FR-010) implemented; FR-011–FR-018 implemented and verified against the real `dine-out-backend`; FR-019 (password show/hide toggle) and FR-020 (login/signup loading overlay) implemented 2026-09-02
...
7. **Given** a password field, **when** the user taps its trailing eye icon, **then** the field toggles between masked and plain text (FR-019); the toggle itself never submits the form or changes the field's value.
8. **Given** either mode, **when** the user taps the primary button and FR-010's validation passes, **then** a blocking loading overlay shows ("Logging in..." / "Creating your account...", FR-020) for as long as the request is in flight, and disappears on success (navigation to Profile) or failure (FR-014's inline error).
...
- **FR-008** — Implemented: sidebar lists Home, Search, Categories, Favorites, Notifications — "Payment methods" excluded. "My orders"/"My reservations" removed 2026-09-02 alongside `profile.md`'s US3/US4 (see that spec's changelog) — not a divergence from this spec, both dropped together.
...
- **FR-019** — Implemented: every password field (`AuthForm`) MUST show a trailing eye icon toggling that field's masking (`secureTextEntry`) between hidden and plain text. Local UI state only, one toggle per field, independent of every other field/validation state.
- **FR-020** — Implemented: while the login/signup mutation (FR-012) is in flight, `AuthForm` MUST show a blocking loading overlay (new shared `src/components/ui/LoadingOverlay.tsx` — a non-dismissible centered `Modal` with a spinner and a label) reading "Logging in..." in login mode, "Creating your account..." in signup mode. Sourced directly from the mutation's own `isPending` (`app/login.tsx`'s existing `isSubmitting`, already threaded into `AuthForm`) — no separate state to keep in sync.
...
| 2026-09-02 | Live-device testing feedback: password fields had no way to reveal what was typed. `AuthForm.tsx`'s password `TextInput` gained a trailing eye icon (new local `showPassword` state, toggling `secureTextEntry`) — added FR-019, US1 scenario 7. Same-repo `profile.md`/this spec's FR-008 also updated this round to drop the removed "My orders"/"My reservations" sidebar entries (see `profile.md`'s changelog for the reasoning — a separate, unrelated fix bundled in the same batch). |
| 2026-09-02 | Live-device testing feedback (separate round, same day): tapping "Log in"/"Create account" gave no feedback while the request was in flight — the button just sat there (dimmed via existing `opacity-60`, but easy to miss), reading as "did nothing happened." Added FR-020 and US1 scenario 8: new shared `src/components/ui/LoadingOverlay.tsx` (non-dismissible centered `Modal`, spinner + label, no `onRequestClose` dismiss path — reflects a request already underway, not a cancelable choice) wired into `AuthForm` off the existing `isSubmitting` prop, no new state. New tests: `src/components/ui/__tests__/LoadingOverlay-test.tsx`, `src/features/auth/components/__tests__/AuthForm-test.tsx` (first dedicated test file for this component). `npx tsc --noEmit`, `npx biome lint .`, `npx jest` (81 tests) clean. |
