...
**Created**: 2026-08-13
**Status**: `Restaurant` entity family modeled and implemented (`dine-out-backend`'s Overture-backed catalog, 41,205 São Paulo rows). A field-enrichment extension (10 additional Overture-sourced columns) is specced below, not yet migrated — see `dine-out-backend`'s `specs/restaurants.md` FR-015–FR-022. A cuisine taxonomy extension (`cuisineId`, replacing the wire contract's frontend-derived `cuisine` — see the persisted-model-vs-wire-contract table below) is likewise specced, not yet migrated — FR-023–FR-027.
...
**Scope**: only the `Restaurant` entity family — the one entity fully implemented and stable (`restaurant.md`'s US1-6). `User`, `Order`, `OrderItem`, `Reservation`, `PaymentMethod`, `PaymentTransaction`, `NotificationSetting`, `Favorite` are not modeled in field-level detail here — see "Deferred entities."
...
- `cuisineId` is likewise Overture-`category`-derived and ingestion-computed, not product-authored: a pure mapping function resolves each of the 122 whitelisted `category` values onto one of 11 buckets (`dine-out-backend`'s `specs/restaurants.md` FR-023), stored and indexed rather than re-derived per request. Specced, not yet migrated (FR-023–FR-024).
...
| `cuisineId` | string, indexed | One of 11 real cuisine buckets, computed from `category` at ingestion time (not per-request) — `restaurant` is the catch-all for a category with no more specific bucket fit. Replaces `GET /taxonomies`'s 5 arbitrary cuisines. Specced (`dine-out-backend`'s `specs/restaurants.md` FR-023–FR-024), not yet migrated. `GET /restaurants?cuisine=X` (`dine-out-backend`'s `specs/restaurants.md` FR-025) filters on it, exact match, combinable with `category`/`occasion`. |
...
| `categoryAlternates` | string[] | Overture `categories.alternate` — a place's secondary category tags, distinct from `category` above. Specced (FR-016), not yet migrated. `GET /restaurants?category=X` does not match this field — exact match against `category` only (`dine-out-backend`'s `specs/restaurants.md` FR-021). |
...
| `brandName`, `brandWikidataId` | string, nullable | Overture `brand.names.primary`/`brand.wikidata` — populated only for rows resolved to a known chain. Specced (FR-018), not yet migrated. Capture-only this round — no `?brand=X` filter (`dine-out-backend`'s `specs/restaurants.md` FR-022). |
...
- **`Order`, `OrderItem`, `Reservation`, `PaymentMethod`, `PaymentTransaction`, `NotificationSetting`** — back `profile.md`'s User Stories 3-6, still P3, not started. Marked `provisional` in `ARCHITECTURE.md` §8. `OrderItem`/`PaymentTransaction` are line-item and charge-attempt tables under `Order`, not independent user stories.
...
| 2026-08-18 | Deferred-entities list updated: `OrderItem` and `PaymentTransaction` added to `ARCHITECTURE.md` §8's schema sketch (order line items and payment charge attempts, respectively). No change to this document's own scope or field-level detail — both remain gated behind `profile.md` US3/US5 same as `Order`/`PaymentMethod`. |
...
| 2026-08-25 | User resolved both open product decisions: `category` filtering stays exact-match only, not broadened to `categoryAlternates` (FR-021); no `?brand=X` filter this round (FR-022). Unblocks implementation. |
| 2026-08-26 | `Restaurant` gains `cuisineId`, a new indexed, ingestion-computed column mapping `category`'s 122 raw Overture values onto 11 real cuisine buckets — replaces `GET /taxonomies`'s 5 arbitrary cuisines and supersedes the "persisted model vs. wire contract" table's prior "not stored" note for frontend-derived `cuisine`. `brazilian_restaurant` (~9.4% of the catalog alone) becomes its own top-level bucket rather than folding into a broader "Latin American" grouping. Design specced in `dine-out-backend`'s `specs/restaurants.md` (FR-023–FR-027), not yet migrated; three open product decisions block implementation. |
| 2026-08-27 | Added `Restaurant.photoUrl` (nullable stock-photo fallback, ingestion-assigned once per row from its cuisine bucket's pool, never reassigned — `dine-out-backend-overture`'s `specs/restaurants.md` FR-028–FR-031). |
