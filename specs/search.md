...
- **Bottom tab bar is confirmed real UI, but inconsistently applied**: the design now repeats an identical 4-item tab bar (Home, Buscar, Categorias, Perfil — no Favoritos, consistent with `favorites.md`'s "no route of its own" decision) across three frames: "1 · Home", "7 · Category page", "6 · Profile" — only the active item's styling differs between copies. The "3 · Search & Map" frame does **not** include it. **[NEEDS CLARIFICATION: should Search & Map also render the persistent tab bar (visible under its draggable results sheet), or is its absence there deliberate — an immersive, tab-chrome-free map screen? Not resolved here.]**
