...
Restaurant ownership claims and edit rights are unaffected by this change — §9 still governs who may set `occasion`/`ambient`/`tags`/`whatsapp`/`instagramHandle`, which are always product-authored regardless of catalog source. An ingested-but-unclaimed row starts with `occasion`/`ambient` unset — see `dine-out-backend`'s `specs/restaurants.md` FR-009 [NEEDS CLARIFICATION].
...
- **Unclaimed-restaurant `occasion`/`ambient`**: Overture ingestion cannot populate these — they are product-authored, set only via an ownership claim (§9). What the app shows for an ingested-but-unclaimed row is unresolved. See `dine-out-backend`'s `specs/restaurants.md` FR-009 `[NEEDS CLARIFICATION]`.
- **Ingestion confidence floor**: unresolved coverage-vs-precision trade-off for the Overture ingestion script. See `dine-out-backend`'s `specs/restaurants.md` FR-014 `[NEEDS CLARIFICATION]`.
