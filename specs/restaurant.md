...
- **Mocks**: `src/mocks/restaurantDetails.ts`, keyed by restaurant `id`, distinct from `src/mocks/restaurants.ts` (which only carries the summary shape used by list/card contexts) — fixture data, served over HTTP via `src/mocks/handlers/restaurantDetails.ts` (MSW), returning 404 for an unknown id. `useRestaurantDetailQuery(id)` calls `apiClient.get('/restaurants/:id')`, validates against `RestaurantDetailSchema`, and maps a 404 specifically to `null` (not a thrown error) so the existing "restaurant not found" UI keeps working — see `PROJECT.md`'s MSW/testing infrastructure entry for the `ApiError` pattern this relies on.
