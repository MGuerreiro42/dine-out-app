...
- **US6 (Similar Places) needed no new wire-contract or mock work** — Google Places has no "similar places" field on Place Details, and rather than inventing one, `app/restaurant/[id].tsx` reuses `useRestaurantsQuery` (from `features/search/api`, the same hook that powers Home) and filters client-side (same `cuisine`, excluding the current `id`, capped at 3). This is a route-level composition of two features' hooks, not a `features/restaurant/` → `features/search/` import — the isolation rule (#1 in `PROJECT.md`) is about features not importing each other directly; `app/` route files are explicitly the composition layer, and the Home route already does the same thing (imports from `features/search/hooks`). Both screens share the same `queryKey: ['restaurants']`, so TanStack Query serves this from cache for free when navigating from Home. New `SimilarPlacesSection` component wraps the reused `HorizontalRail`/`RestaurantCard`, returns `null` if there are zero candidates (same "hide if nothing to show" precedent as `AmenitiesSection`/`ReviewsSection`).
...
| 2026-07-24 | User Story 6 implemented (Similar Places rail). New component `SimilarPlacesSection`, reusing `RestaurantCard`/`HorizontalRail` exactly as this Architecture Mapping already anticipated. No new mocks or wire-contract work — `app/restaurant/[id].tsx` reuses `useRestaurantsQuery` (from `features/search/api`) and filters client-side by cuisine, a route-level composition rather than a features-importing-features violation. Navigation uses `router.replace`, not `push`, per acceptance scenario 2's "replacing the current detail, not stacking indefinitely." User Story 7 (favorite) is the only one left, still blocked on `stores/favorites.ts`. |
...
| 2026-08-17 | **Status/FR correction** (found during `favorites.md`'s User Story 2 design pass, not implemented here): `feat/restaurant-detail-redesign` (commit `e3da6e0`, 2026-08-12) implemented User Story 7/FR-016 (the like icon, via `DetailHeaderActions.tsx` and independently on `RestaurantCard.tsx`) without this spec ever being updated to say so. The same commit simplified the header overlay to share + favorite only, dropping the location/settings/profile icons — FR-019, FR-020, and FR-021 are no longer present in the shipped UI even though still marked "Implemented" above; marked Superseded rather than rewritten, since a full audit of acceptance scenario 5 / the Architecture Mapping's icon-stack notes is a separate pass. FR-022 (share icon) is unaffected — still shipped as described. |
